...
<mission_name>で指定できる引数は以下のとおりです。
  ...
  * - 引数
    - 詳細
  * - M3-A1
    - 練習環境（エリア1のみ）を読み込みます。デフォルトでAizuSpiderDSとHobbyDroneを配置します。
  * - M3-A2
    - 練習環境（エリア2のみ）を読み込みます。デフォルトでAizuSpiderDSとHobbyDroneを配置します。
  * - M3-A3
    - 練習環境（エリア3のみ）を読み込みます。デフォルトでAizuSpiderDSとHobbyDroneを配置します。
  * - M3-A4
    - 練習環境（エリア4のみ）を読み込みます。デフォルトでAizuSpiderDSとHobbyDroneを配置します。

練習環境に自作のロボットを配置したい場合は、以降に記述する **競技環境の読込（応用編）** の作業を行い、WRS2025/registration以下に格納されているregistration_m3.yamlを直接またはコピーしてから **robot_list** のパラメータを編集してください。コピーした場合は、Choreonoidを起動するときの引数でコピーしたファイルを指定するようにしてください。
...
<mission_name>で指定できる引数は以下のとおりです。
  ...
  * - 引数
    - 詳細
  * - PS-1
    - 競技環境（エリア1のみ）を読み込みます。デフォルトでAizuSpiderDSとHobbyDroneを配置します。
  * - PS-2
    - 競技環境（エリア2のみ）を読み込みます。デフォルトでAizuSpiderDSとHobbyDroneを配置します。
  * - PS-3
    - 競技環境（エリア3のみ）を読み込みます。デフォルトでAizuSpiderDSとHobbyDroneを配置します。
  * - PS-4
    - 競技環境（エリア4のみ）を読み込みます。デフォルトでAizuSpiderDSとHobbyDroneを配置します。
...
<mission_name>で指定できる引数は以下のとおりです。
  ...
  * - 引数
    - 詳細
  * - PS-1
    - 競技環境（エリア1のみ）を読み込み、自作のロボットモデルとHobbyDroneを配置します。
  * - PS-2
    - 競技環境（エリア2のみ）を読み込み、自作のロボットモデルとHobbyDroneを配置します。
  * - PS-3
    - 競技環境（エリア3のみ）を読み込み、自作のロボットモデルとHobbyDroneを配置します。
  * - PS-4
    - 競技環境（エリア4のみ）を読み込み、自作のロボットモデルとHobbyDroneを配置します。
