...
ChoreonoidをROS2環境で使用している場合は、 ::
 ...
 $ ros2 run choreonoid_ros choreonoid ~/ros2_ws/src/choreonoid/ext/WRS2025/registration/registration_test2.yaml --wrs-util <mission_name>

Choreonoidを単体で使用している場合は、（例：choreonoid/build内で） ::

 $ ./bin/choreonoid ../ext/WRS2025/registration/registration_test2.yaml --wrs-util <mission_name>
...
<mission_name>で指定できる引数は以下のとおりです。
  ...
  * - 引数
    - 詳細
  * - M2
    - 競技M2の環境（エリア1〜4）を読み込みます。デフォルトでMonoCrawlerNAとHobbyDroneを配置します。
  * - M2-A1
    - 競技M2の環境（エリア1とエリア2のみ）を読み込みます。デフォルトでMonoCrawlerNAとHobbyDroneを配置します。
  * - M2-A2
    - 競技M2の環境（エリア2とエリア3のみ）を読み込みます。デフォルトでMonoCrawlerNAとHobbyDroneを配置します。
  * - M2-A3
    - 競技M2の環境（エリア3とエリア4のみ）を読み込みます。デフォルトでMonoCrawlerNAとHobbyDroneを配置します。
  * - M2-A4
    - 競技M2の環境（エリア4とエリア1のみ）を読み込みます。デフォルトでMonoCrawlerNAとHobbyDroneを配置します。
...
ここでは、WRS2025/registration以下に格納されているregistration_test2.yamlをテンプレートとして利用します。
registration_test2.yamlを任意のディレクトリにコピーして、任意の名前（例：registration_<チーム名>.yaml）に変更してください。
