...
「メニュー」-「KIOSK」-「ログの保存」にチェックを入れると、シミュレーション実行時にワールドログファイルを自動的に生成・保存します。保存したワールドログファイルは、プロジェクトをセットで保存され、KIOSKビューの「ログ」タブに登録されます。保存されたワールドログファイル及びプロジェクトは、Choreonoidのビルドディレクトリのshare/choreonoid-x.x/kiosk/log以下に格納されています。
