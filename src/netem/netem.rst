...
ボディNetEmコントローラアイテムの作成と設定
-------------------------------------------

まずボディNetEmコントローラアイテムを作成します。シミュレーション中に通信を行うボディをアイテムツリービューで選択して、続けてメインメニューの「ファイル」-「新規」-「ボディNetEmコントローラ」を選択して生成してください。
生成したボディNetEmコントローラアイテムが選択したボディの子アイテムとして配置されます。シミュレーション中に通信を行うボディが複数ある場合には、ボディ毎にボディNetEmコントローラアイテムを1つずつ子アイテムとして配置します。

次にボディNetEmコントローラアイテムのパラメータを設定します。ボディNetEmコントローラアイテムのパラメータは以下の通りです。
...
.. list-table::
  ...
  * - 送信先IPアドレス
    - \-
    - \-
    - パケットの送信先のIPアドレスを指定します。ボディNetEmコントローラアイテムを配置したボディが通信を行う任意の送信先のIPアドレスを入力してください。また、IPアドレスの後ろにはプレフィックス帳を必ず付与してください。具体的には、xxx.xxx.xxx.xxx/32のように指定します。ここでは/32として指定したIPアドレスのみを送信先に指定しています。

以上でボディNetEmコントローラアイテムの設定は完了です。
